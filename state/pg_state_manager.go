...
func (sm *SQLStateManager) UpdateRun(runID string, updates Run) (Run, error) {
	...
	for rows.Next() {
		err = rows.Scan(
			...
			&existing.PodEvents, &existing.CommandHash, &existing.ExecutableID, &existing.ExecutableType)
	}
	if err != nil {
		return existing, errors.WithStack(err)
	}
    ...
    UPDATE task SET
	  ...
	  command = $17, memory = $18, cpu = $19, gpu = $20, engine = $21, ephemeral_storage = $22, node_lifecycle = $23,
		...
		executable_id = $30, executable_type = $31
    WHERE run_id = $1;
    `
	...
	if _, err = tx.Exec(
		...
		existing.MaxMemoryUsed, existing.PodEvents, existing.ExecutableID, existing.ExecutableType); err != nil {
		tx.Rollback()
		return existing, errors.WithStack(err)
	}
